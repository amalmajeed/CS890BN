...
def message_split(m,size):
	...
	for i in range(0,lim):
		if(i != (lim-1)):
			...
			d["data"] = m[i*size:]
			if(flag):
				print "\nThe last block contains : ",len(d["data"])
				rem = 252 - len(d["data"])
				d["data"]+= "1"
...
def sender_logic(m,S_sen):
	...
	print "\n\t\tFinal Encypted packets list from the sender side : \n",enc_list,"\n"
	print "\n\t\t\t\t\tEncryption Ends Here\t\t\t\t\t\t\n"
	return enc_list
...
def receiver_logic(seq_list,enc_list,S_rec):
	...
	for x in seq_list:
		...
		if(check_val < 0):
			...
			skip = (check_val)*268
			# 'skip' gives the number of bytes we are ahead of which is the number of IPRGA iterations needed to be in sync with the packet received
			for z in range(0,skip):
